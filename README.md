...
The startup script watches connected ADB devices and restores forwarding within about three seconds after a cable reconnect, in both USB and LAN modes. It supports Windows PowerShell 5.1 and PowerShell 7: it uses Start-ThreadJob when available and otherwise the built-in Start-Job. The watcher stops when the backend script exits. If the watcher cannot start, the backend still runs and prints the manual forwarding command. Select USB, wait briefly, then tap Reconnect. ADB must be on the PC's PATH and the device must authorize USB debugging. If starting Node directly, run `./scripts/watch-adb.ps1` in another terminal, or use the manual forwarding command below.

If PowerShell reports that running scripts is disabled, run `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass` in the same terminal, then retry the startup command. This setting lasts only for that terminal session; organization-enforced policies may still apply.
